...
#
# Put the novetus launcher, launch scripts and default map inside the container
#
...
#
# Setup wineprefix, then create some files to slient a few warnings.
#
...
RUN export WINEPREFIX=/home/novetus/.wine && \
    ...
    wineserver -k && \
    wineboot -u && \
    touch /Launcher/data/config/servers.txt /Launcher/data/config/ports.txt
